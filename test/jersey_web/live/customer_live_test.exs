defmodule JerseyWeb.CustomerLiveTest do
  ...
  describe "Index" do
    setup [:create_customer, :create_city]
    ...
    test "saves new customer", %{conn: conn, city: city} do
      ...
      assert html =~ dgettext("customer", "New Customer")
    ...
    test "updates customer with city selection in listing", %{conn: conn, customer: customer} do
      {:ok, index_live, _html} = live(conn, ~p"/customers")
  ...
  describe "Show" do
    ...
    test "updates customer with city selection and returns to show", %{
      conn: conn,
      customer: customer
    } do
      {:ok, show_live, _html} = live(conn, ~p"/customers/#{customer}")
